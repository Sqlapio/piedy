    ...
    protected function getStats(): array
    {
        ...
        return [
            ...
            Stat::make('TOTAL SERVICIOS REALIZADOS', array_sum($count_servicios))
                ...
                ->color('primary')
                ->extraAttributes(['class' => 'col-span-3 row-span-1 rounded-md text-center border-4 border-[#3ec7d28a]' ]),

            Stat::make('TOTAL CLIENTES ATENDIDOS', count($clientes))
                ->description('Total de clientes atendidos')
                ->descriptionIcon('heroicon-m-user-group')
                ->color('info')
                ->extraAttributes(['class' => 'col-span-3 row-span-1 rounded-md text-center border-4 border-[#7B9AA6]']),

            Stat::make('TOTAL PAGOS EN USD($)', '$' . $this->getPageTableQuery()->sum('pago_usd'))
                ->description('Pago total en USD($)')
                ->descriptionIcon('heroicon-m-currency-dollar')
                ->color('success')
                ->extraAttributes(['class' => 'col-span-3 row-span-1 rounded-md text-center border-4 border-[#bf9c999e]']),

            Stat::make('TOTAL PAGOS EN BS.', 'BS.' . $this->getPageTableQuery()->sum('pago_bsd'))
                ->description('Pago total en Bs')
                ->descriptionIcon('heroicon-m-arrow-trending-up')
                ->color('warning')
                ->extraAttributes(['class' => 'col-span-3 row-span-1 rounded-md text-center border-4 border-[#9bad699e]']),
