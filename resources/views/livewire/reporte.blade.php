<div>
    ...
    <div class="px-10 py-4">
        <div class="mb-10">
            <div class="flex">
                <h1 class="text-xl mb-6 font-extrabold text-[#7898a5] uppercase">MODULO DE REPORTES</h1>
            </div>
            ...
            <div class="grid md:grid-cols-4 sm:gap-4 md:gap-4 mb-40">
                <!-- Empleado -->
                <div class="">
                    <x-select wire:change="$emit('selected', $event.target.value)" label="Empleado" wire:model.defer="empleado" placeholder="Seleccion" :async-data="route('api.empleados.nomina')" option-label="name" option-value="id" :disabled='$atr_disabled == 1'/>
                </div>
                <!-- Periodo de nomina -->
                <div>
                    <x-select wire:change="$emit('selected', $event.target.value)" label="Periodo de Nomina" wire:model.defer="periodo" placeholder="Seleccion" :async-data="route('api.periodo.nomina')" option-label="cod_quincena" option-value="cod_quincena" :disabled='$atr_disabled == 1'/>
                </div>
                <!-- Boton -->
                <div class="mt-auto">
                    <button type="submit" wire:click.prevent="reportes()" class="w-full rounded-md border border-transparent bg-green-700 py-2 px-4 text-sm font-bold text-white shadow-sm hover:bg-check-green" :disabled='$atr_disabled == 1'>
