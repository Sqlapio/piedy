<div>
    ...
    <div class="p-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-6 gap-4 mb-4 mt-8 {{ $ocultar }}">
            {{-- Descripcion --}}
            <div class="p-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-6 gap-4 mb-4 mt-8 {{ $ocultar }}">
            ...
            <div class="p-2">
                <label class="opacity-60 mb-1 block text-sm font-medium text-italblue">Empleado</label>
                <x-select wire:model.defer="empleado_id" placeholder="Seleccion" :async-data="route('api.empleados')" option-label="nombre" option-value="id" />
            </div>
            <div class="p-2">
        ...
        <div class="flex justify-end p-2 mt-auto mb-4 {{ $botton_agendar_cita }}">
            <button type="submit" wire:click.prevent="mostrar()" class="justify-end rounded-md border border-transparent bg-[#7898a5] py-2 px-4 text-sm font-bold text-white shadow-sm hover:bg-check-green">
                <svg xmlns="http://www.w3.org/2000/svg" wire:loading wire:target="store" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="animate-spin h-5 w-5 mr-3">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" />
                </svg>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="bg-white p-4 rounded-lg shadow" style="background-image: url('https://wallpaper-house.com/data/out/8/wallpaper2you_256843.jpg'); background-size: cover;">
                ...
                {{-- <div class="flex items-center justify-start mb-6">
                    ...
                    <div class="text-md text-black">Cliente: {{ $item->get_cliente->nombre }} {{ $item->get_cliente->apellido }}</div>
                </div>
                ...
                <div class="text-right">
                    ...
                    <div class="mt-2 text-2xl text-black leading-7 font-bold">
                        Cliente: {{ $item->get_cliente->nombre }} {{ $item->get_cliente->apellido }}
                    </div>
