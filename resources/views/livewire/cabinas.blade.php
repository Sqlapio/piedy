        ...
        @foreach ($data as $item)
            <div id="accordion-collapse" data-accordion="collapse" class="w-full">
                <h2 id="accordion-collapse-heading-{{ $item->id }}">
                    <button  type="button" class="flex justify-start items-center w-full p-5 {{ $item->status == 'activo' ? 'text-white' : 'text-black' }} font-extrabold rounded-xl {{ $item->status == 'activo' ? 'bg-green-700' : 'bg-yellow-300' }} gap-3" data-accordion-target="#accordion-collapse-body-{{ $item->id }}" aria-expanded="false" aria-controls="accordion-collapse-body-{{ $item->id }}">
                        <img class="w-20 h-auto rounded-full ml-4" src="{{ $item->status == 'activo' ? asset('images/abierto.png') : asset('images/cerrado2.png') }}" alt="">
